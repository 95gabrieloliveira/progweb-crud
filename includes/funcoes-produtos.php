...
function inserirProduto($conexao, $nome, $preco, $quantidade, $descricao, $fabricanteId){
    $sql = "INSERT INTO produtos(nome, preco, quantidade, descricao, fabricante_id) VALUES ('$nome', $preco, $quantidade, '$descricao',
     $fabricanteId)";
...
function atualizarProduto($conexao, $id, $nome, $preco , $descricao, $quantidade, $fabricanteId ){
    $sql = "UPDATE produtos SET nome = '$nome', preco = '$preco', descricao ='$descricao', fabricante_id = $fabricanteId WHERE id = $id";
    mysqli_query($conexao, $sql) or die(mysqli_error($conexao));
